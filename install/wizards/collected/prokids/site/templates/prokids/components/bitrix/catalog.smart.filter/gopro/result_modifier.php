...
foreach($arResult['ITEMS'] as $key => $arItem)
{
	...
	if(in_array($arItem['CODE'],$arParams['FILTER_PRICE_GROUPED']))
	{
		...
		if($arItem['PROPERTY_TYPE']=='N' || isset($arItem['PRICE'])) {
			$minCalculated = CollectDevLibFilterExtension::RoundCustom($arItem['VALUES']['MIN']['VALUE'], CollectDevLibFilterExtension::GetTo4round($arItem['VALUES']['MIN']['VALUE']), 'big');
			$maxCalculated = CollectDevLibFilterExtension::RoundCustom($arItem['VALUES']['MAX']['VALUE'], CollectDevLibFilterExtension::GetTo4round($arItem['VALUES']['MAX']['VALUE']), 'small');
			$putKey = false;
